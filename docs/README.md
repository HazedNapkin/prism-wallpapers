...
### Missing or Distorted Brand Folders
* **Issue:** The script runs but doesn't output any files or prints an error about directories.
* **Fix:** Ensure your folder structure strictly matches the required layout. The script looks for `.png` files directly inside `collections/networks/<brand_name>/logos/color/` and `white/`.

### Logo Appears Off-Center
* If a new logo looks visually off-center due to an asymmetrical design, locate its unique ID in the filename and add a manual entry to the `DESIGN_HOTFIXES` dictionary at the top of `logo_cards.py`.

### Image Compression or Quality Issues
* The script uses high-quality `Image.Resampling.LANCZOS` downscaling. If your output appears pixelated, ensure your source logo files have a resolution of at least `1080px` on the shortest side before running the script.


## 📂 Asset Pipeline

The script depends on a strict input structure. Below is a blueprint of how raw assets are processed and where outputs are stored.
...
collections/
 ...
 │    └── 6219_mgm_plus/
 │         ├── logos/
 │         │    ├── color/ ──> [mgm_color.png] (Raw Input)
 │         │    └── white/ ──> [mgm_white.png] (Raw Input)
 │         │
 │         └── cards/  <─── [Automatically Generated Output]
 │              ├── transparent/
 │              └── dual_2d1d2d_231a3a_0e0914/ ──> [card_mgm_color.png]
 ```
